...
filename = f"/storage/emulated/0/alpha_advanced/Alpha_advanced/backtest_results.csv"

current_time = datetime.datetime.now().time()
df = sdtools.intra_data(security_id=694 ,interval=5)
entry_df = sdtools.backtest_entries(df)
entry_df.to_csv(filename, index=False)
print(df)
print(entry_df)
# pdb.set_trace()
focused_opts = ["HEROMOTOCO"]
                
main_list = ["ABB","ADANIENSOL", "ADANIENT", "ADANIGREEN", "ADANIPORTS", "APLAPOLLO", "APOLLOHOSP", "ASIANPAINT", "ASTRAL", "ATGL", "AUROPHARMA", "AXISBANK", "BAJAJ-AUTO", "BAJAJFINSV", "BAJFINANCE", "BHARATFORG", "BHARTIARTL", "BPCL", "BSE", "CAMS", "CDSL", "CGPOWER", "CHAMBLFERT", "CHOLAFIN", "CIPLA", "COFORGE", "CUMMINSIND","DIVISLAB", "DIXON", "DLF","DRREDDY", "EICHERMOT", "ETERNAL", "GLENMARK", "GODREJPROP", "GRASIM", "HAL", "HAVELLS", "HCLTECH", "HDFCBANK", "HEROMOTOCO", "HINDALCO", "HINDUNILVR", "ICICIBANK", "INDIGO", "INDUSINDBK", "INFY", "JINDALSTEL", "JSWSTEEL", "JUBLFOOD", "KAYNES", "KFINTECH", "KOTAKBANK", "LT", "LTIM", "LUPIN", "M&M", "MARUTI", "MPHASIS", "MUTHOOTFIN", "NAUKRI", "OBEROIRLTY", "OFSS", "PERSISTENT", "PETRONET", "POLYCAB", "RELIANCE", "SBICARD", "SBILIFE", "SBIN", "SIEMENS",  "SUNPHARMA", "TCS", "TECHM", "TITAN", "TRENT", "TVSMOTOR", "VEDL", "VOLTAS"]
...
while True:
    ...
    elif datetime.time(9, 31) <= current_time < datetime.time(12, 50):
        print("🚀 Scan begins")
        ...
        if bullish_trend == True:
            ...
            try:
                for stock_name, stock_data in open_low_dict.items():
                    ...
                    stock_id = stock_data["security_id"]
                    direction = "CALL"
                    Call_entry = sdtools.sd_check_call_entry1(stock_name, stock_id)
                    if Call_entry == True:
            ...
            try:
                for stock_name, stock_data in open_high_dict.items():
                    ...
                    stock_id = stock_data["security_id"]
                    direction = "PUT"
                    Put_entry = sdtools.sd_check_put_entry1(stock_name, stock_id)
                    if Put_entry == True:
